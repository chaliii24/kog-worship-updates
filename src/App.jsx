...
import { formatCountdown, TRANSITIONS, TRANSITION_KEYS, SPEED_OPTIONS, FONT_OPTIONS } from './lib/constants';
import { emphasisLine, applyCaseTransform, renderLyricsLayout } from './lib/lyrics';
...
export default function App() {
  ...
  useEffect(() => {
    if (isOutputWindow || !window.require) return;
    const { ipcRenderer } = window.require('electron');
    const onUpdateAvailable = (event, info) => setAboutStatus(`Update available: v${info.version}`);
    const onUpdateDownloaded = (event, info) => setAboutStatus(`Update downloaded: v${info.version}. Restart to install.`);
    const onUpdateError = (event, error) => setAboutStatus(`Update error: ${error}`);
    const onUpdateNotAvailable = (event, info) => setAboutStatus(`You're running the latest version (v${info.version}).`);
  ...
  useEffect(() => {
    try { localStorage.setItem('scriptureBgLibrary', JSON.stringify(scriptureBgLibrary)); } catch {}
  }, [scriptureBgLibrary]);
  ...
  }, [handleNextCue, handlePrevCue, showModalOpen, showBuilder, builderSheet, builderTileIdx, dockTab, bibleStep]);
...
// --- STAGE DISPLAY FEED ---
  ...
  const openNewShow = () => {
    setShowBuilder({ name: 'Sunday Show', date: new Date().toISOString().split('T')[0], category: 'Worship', ratio: '16:9', resolution: '1920x1080', sections: defaultShowSections() });
    setBuilderSongQuery({});
  ...
  const addShowSection = () => {
    setShowBuilder(prev => ({
      ...prev,
      sections: [...(prev.sections || []), { id: 'sec-' + Date.now() + '-' + ((prev.sections || []).length + 1), title: 'New Section', items: [] }]
    }));
  };
  ...
  const removeShowSection = (sectionId) => {
    if (!confirm('Remove this section and all its slides?')) return;
    setShowBuilder(prev => ({
      ...prev,
  ...
  const createShow = async () => {
    if (!showBuilder || !showBuilder.name.trim()) { alert('Please name your show.'); return; }
    const items = [];
  ...
  const builderGoLive = (entry, tileIdx) => {
    ...
    if (item.songId) {
      ...
      if (tileIdx === 0) {
        setActiveSong(song);
        const titleCue = { id: 'title-card', label: 'Song Title', text: song.title };
        ...
        const effectiveStyle = (() => {
          if (cueHasBackground(cue)) return { ...stageStyle, backgroundType: cue.bg_type || 'color', backgroundValue: cue.bg_value || '#000000', lyric: cueLyricStyle(cue) };
  ...
  const addShowBuilderMedia = async (e) => {
    ...
    const secIdx = builderSheet?.secIdx ?? Math.max(0, (showBuilder?.sections?.length || 1) - 1);
    const secId = showBuilder?.sections?.[secIdx]?.id;
    if (secId) {
      setShowBuilder(prev => ({
        ...prev,
  ...
  const fireTitleLive = () => {
    ...
    liveBibleRef.current = null;
    liveBibleSrcRef.current = null;
    const titleCue = { id: 'title-card', label: 'Song Title', text: activeSong.title };
    setActiveCue(titleCue);
    setSlideTimer({ start: Date.now(), elapsed: 0, duration: 0 });
    const slidePayload = { 
  ...
  const addSongToService = (song) => {
    ...
    setActiveService({ ...activeService, items: [...activeService.items, newItem] });
  };
  ...
  const clearServiceOrder = () => {
    if (!confirm('Clear the entire service order?')) return;
    setServiceCollapsed([]);
    setActiveService({ ...activeService, items: [] });
  };
  ...
  const fireServiceMediaLive = (item) => {
    ...
    const slidePayload = { title: item.title, text: '', label: item.subtitle || 'Media', style: { ...stageStyle, backgroundType: item.media_type || 'image', backgroundValue: item.media_url }, audio: activeSong?.audio_url || null, timestamp: Date.now() };
  ...
  const processAutoPaste = async (textOverride) => {
    ...
    if (!text || !text.trim()) {
      alert("Please paste a chord chart or lyrics first!");
      return;
    }

    if (selectedAiModel === 'ollama' && text.length > 5000) {
      alert("Text is too long for local Ollama! Please trim your text or switch to Gemini 1.5 Flash.");
      return;
    }
  ...
  const fetchSongFromUrl = async () => {
    if (!importUrl.trim()) { alert('Paste a chord chart or lyrics URL first.'); return; }
  ...
  const onStagePointerMove = (e) => {
    ...
    updateCue(editorCueIdx, { box: { x: Math.round(nx), y: Math.round(ny), w: Math.round(nw), h: Math.round(nh) } });
  };
  ...
  const handleDeleteSong = async (id) => {
    if (window.require && confirm('Are you sure you want to delete this song?')) {
      const { ipcRenderer } = window.require('electron');
  ...
  const removeMediaAsset = async (asset) => {
    if (!window.require || !confirm(`Remove "${asset.url.split('/').pop()}" from the media library?\n\nSongs that reference it will fall back to their color background.`)) return;
    const { ipcRenderer } = window.require('electron');
  ...
  const buildBiblePayload = ({ book, chapter, verse, text }, bgOverride) => {
    const ref = `${book} ${chapter}${verse ? `:${verse}` : ''}`;
    const lines = text.split('\n').map((ln) => {
      let s = ln.trim();
      if (bibleFmt.bold) s = `**${s}**`;
      return s;
    });
    const effectiveBg = bgOverride || bibleMedia;
    const style = effectiveBg
    ...
    const payload = {
      ...
      meta: { kind: 'bible', book, chapter, verse, translation: bibleTrans, source: { book, chapter, verse, text } }
    };
  ...
  const queueBibleServiceSlide = ({ book, chapter, verse, text }) => {
    const item = buildBiblePayload({ book, chapter, verse, text });
    setActiveService({ ...activeService, items: [...activeService.items, item] });
  };

  const fireBibleLive = ({ book, chapter, verse, text }) => {
    fireServiceItemLive(buildBiblePayload({ book, chapter, verse, text }));
  };

  const fireBibleSelectionLive = () => {
    if (!bibleChapter) return;
    const verses = (bibleChapter.verses || []).filter(v => bibleSelVerses.length === 0 || bibleSelVerses.includes(v.verse));
    if (!verses.length) return;
    const combos = verses.map(v => ({ book: bibleChapter.book, chapter: bibleChapter.chapter, verse: v.verse, text: formatBibleVerse(v) }));
    combos.forEach((c, i) => {
      if (i === 0) fireBibleLive(c);
      else setActiveService({ ...activeService, items: [...activeService.items, buildBiblePayload(c)] });
    });
  };
  ...
  const queueBibleSelection = () => {
    if (!bibleChapter) return;
    const verses = (bibleChapter.verses || []).filter(v => bibleSelVerses.length === 0 || bibleSelVerses.includes(v.verse));
    verses.forEach(v => queueBibleServiceSlide({ book: bibleChapter.book, chapter: bibleChapter.chapter, verse: v.verse, text: formatBibleVerse(v) }));
  };
  ...
  // Verse click modes: plain = select one + send live; Ctrl/Cmd = toggle; Shift = contiguous range
  const handleBibleVerseClick = (e, v) => {
    ...
    bibleLastVerseRef.current = num;
    fireBibleLive({ book: bibleChapter.book, chapter: bibleChapter.chapter, verse: num, text: formatBibleVerse(v) });
  };
  ...
  const deleteBibleTranslation = async (abbrev) => {
    if (!window.require) return;
    if (!confirm(`Delete ${abbrev.toUpperCase()} from this device? It must be re-downloaded to use offline.`)) return;
    const { ipcRenderer } = window.require('electron');
  ...
  const resolveBibleReference = async () => {
    ...
    if (res && res.bookIndex) {
      setBibleSearchResults(null);
      setBibleSel({ bookIndex: res.bookIndex, bookName: res.book, chapter: res.chapter, totalChapters: res.totalChapters });
      const ch = await loadBibleChapter(bibleTrans, res.bookIndex, res.chapter);
      if (res.verse) {
        setBibleSelVerses([res.verse]);
        const v = (ch?.verses || []).find(x => x.verse === res.verse);
        if (v) fireBibleLive({ book: res.book, chapter: res.chapter, verse: res.verse, text: formatBibleVerse(v) });
      }
    }
  };
  ...
  const deleteSavedService = async (serviceId) => {
    if (!window.require || !confirm('Delete this saved service plan?')) return;
    const { ipcRenderer } = window.require('electron');
  ...
  const renderOutputPreview = () => {
    const valid = monitorContent && (monitorContent.text || monitorContent.presentation?.slide);
    const st = monitorContent?.style || {};
    return (
      <div ref={setPreviewWrapRef} style={{ pointerEvents: 'none', position: 'relative', width: '100%', aspectRatio: outputAspect.replace(':', ' / '), borderRadius: 12, overflow: 'hidden', background: '#000', border: '1px solid #2b2b44', boxSizing: 'border-box' }}>
        ...
        {monitorContent && monitorContent.presentation?.slide ? (
          <div key={`pres-${monitorContent.timestamp}`} style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
  ...
  const appValue = {
    ...
    serviceAddMenu, setServiceAddMenu, serviceSongQuery, setServiceSongQuery,
    ...
    toggleDevProjectorWindow, toggleStageWindow, addSongToService, addHeaderToService, addCustomSlideToService,
    ...
    clearServiceOrder, addMediaItemToService, fireServiceMediaLive, saveCurrentService, loadService, queueShowIntoService,
