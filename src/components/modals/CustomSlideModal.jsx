...
export default function CustomSlideModal({ C, ACCENT, newSlideData, setNewSlideData, onCancel, onAdd }) {
  return (
    <motion.div {...modalOverlay} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }}>
      <motion.div {...modalPanel} style={{ background: C.panel, border: '1px solid #2d2d3f', borderRadius: '14px', width: '440px', maxWidth: '92vw', maxHeight: 'calc(100vh - 80px)', overflowY: 'auto', padding: '22px', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'grid', gap: '14px', marginBottom: '20px' }}>
          <div>
            ...
            <input type="text" value={newSlideData.title} onChange={(e) => setNewSlideData({ ...newSlideData, title: e.target.value })} style={{ width: '100%', background: C.elevated2, border: '1px solid #2d2d3f', borderRadius: '8px', padding: '10px', color: C.text, fontSize: '14px', outline: 'none' }} />
          </div>
          <div>
            ...
            <input type="text" value={newSlideData.subtitle} onChange={(e) => setNewSlideData({ ...newSlideData, subtitle: e.target.value })} style={{ width: '100%', background: C.elevated2, border: '1px solid #2d2d3f', borderRadius: '8px', padding: '10px', color: C.text, fontSize: '14px', outline: 'none' }} />
          </div>
          <div>
            ...
            <textarea rows={4} value={newSlideData.content} onChange={(e) => setNewSlideData({ ...newSlideData, content: e.target.value })} placeholder="Type announcement or scripture text..." style={{ width: '100%', background: C.elevated2, border: '1px solid #2d2d3f', borderRadius: '8px', padding: '10px', color: C.text, fontSize: '14px', outline: 'none', resize: 'vertical' }} />
          </div>
        </div>
