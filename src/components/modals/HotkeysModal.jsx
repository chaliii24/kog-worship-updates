...
export default function HotkeysModal({ C, ACCENT, onClose }) {
  return (
    <motion.div {...modalOverlay} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }} onClick={onClose}>
      <motion.div {...modalPanel} onClick={(e) => e.stopPropagation()} style={{ background: C.panel, border: '1px solid #2d2d3f', borderRadius: '14px', width: '420px', maxWidth: '92vw', maxHeight: 'calc(100vh - 80px)', overflowY: 'auto', padding: '22px', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'grid', gap: '8px', marginBottom: '22px' }}>
          {SHORTCUTS.map(([key, desc]) => (
            <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: C.elevated2, border: '1px solid #2d2d3f', borderRadius: '8px', padding: '10px 14px' }}>
