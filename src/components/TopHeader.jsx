...
export default function TopHeader({ C, PINK, activeMenu, setActiveMenu, menuItems, activeCue, toggleDevProjectorWindow, toggleStageWindow, openNewShow }) {
  ...
  return (
    <motion.div initial={{ y: -16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: 'spring', stiffness: 280, damping: 26, delay: 0.05 }} style={{ background: C.panel, padding: '0 14px', height: 54, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #23233a', gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 2, position: 'relative' }}>
          {['file', 'edit', 'view', 'help'].map(m => (
            <div key={m} style={{ position: 'relative' }}>
              ...
              {activeMenu === m && (
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 400, damping: 28 }} style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, background: C.elevated, border: '1px solid #2b2b44', borderRadius: 10, padding: 5, display: 'grid', gap: 2, minWidth: 180, zIndex: 300, boxShadow: '0 14px 34px rgba(0,0,0,0.7)' }}>
      ...
      {/* RIGHT QUICK ACTIONS */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 10px', background: isLive ? 'rgba(34,197,94,0.1)' : C.elevated2, border: isLive ? '1px solid rgba(34,197,94,0.35)' : '1px solid #2b2b44', borderRadius: 999 }}>
        ...
        <motion.button {...stubTap} onClick={toggleDevProjectorWindow} title="Projector outputs" style={{ background: C.elevated2, border: '1px solid #2b2b44', color: C.heading, padding: '6px 10px', borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
          <Monitor size={13} /> Outputs
        </motion.button>
        <motion.button {...stubTap} onClick={toggleStageWindow} title="Stage display" style={{ background: C.elevated2, border: '1px solid #2b2b44', color: C.heading, padding: '6px 10px', borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
